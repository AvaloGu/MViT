...
from loader.final_loader import MixupCutmixAugmentation, create_dali_loader, build_file_lists, combine_multi_reader_batch, clip_spatial_aug
...
def train_one_epoch(model, loader, n_readers, mixup_cutmix, optimizer, lr_scheduler, criterion, device, steps_per_epoch):
    model.train()
    ...
    for step in range(steps_per_epoch):
        ...
        try:
            ...
            # DALI's notion of epoch ended, auto_reset handles it
            loader_iter = iter(loader)
            data = next(loader_iter)

        aug1, aug2, label = combine_multi_reader_batch(data, n_readers) # (B/2, T, C, 224, 224), (B/2,)
        aug1 = clip_spatial_aug(aug1)
        aug2 = clip_spatial_aug(aug2)

        assert label.dtype == torch.long
        assert label.min() >= 0, label.min().item()
        assert label.max() < 400, label.max().item()
        ...
        # x, y = x.to(device, non_blocking=True), y.to(device, non_blocking=True) # (B/2, 2, T, C, 224, 224), (B/2,)
        ...
        if y.dtype == torch.long:  # hard labels path
            assert y.max() < 400 and y.min() >= 0
        else:  # soft labels from mixup
            assert y.shape == (logits.shape[0], 400)
...
def evaluate(model, loader, criterion, device, n_readers):
    model.eval()
    ...
    with torch.no_grad():
        for data in loader:
            ...
            aug1, aug2, y = combine_multi_reader_batch(data, n_readers) # (B/2, T, C, 224, 224), (B/2,)
            aug1 = clip_spatial_aug(aug1)
            aug2 = clip_spatial_aug(aug2)
...
def train():
    ...
    paths, labels = build_file_lists(csv_path, video_dir)
    train_loader = create_dali_loader(filenames=paths, labels=labels, batch_size=32, num_threads=14, n_readers=n_readers, device_id=0)
    ...
    lr_warmup_iters = 50 * iters_per_epoch
    ...
    for epoch in range(num_epoch):
        ...
        if epoch == num_epoch - 1:
            val_loader = create_dali_loader(filenames=paths, labels=labels, batch_size=32, num_threads=14, n_readers=n_readers, device_id=0)
            val_loss, total_correct, total_samples = evaluate(model, val_loader, criterion_val, device, n_readers)
